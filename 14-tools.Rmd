...
But how do you go about determining if a paper has been retracted? There are website you can check like (retraction watch)[https://retractionwatch.com]. It can, however, be time consuming to check for every article in this website. 

An easier approach is use the [http://openretractions.com](http://openretractions.com) website where you type in the DOI number from an article and it checks if that article has been retracted. Even better, you can use the [retractcheck](https://cran.r-project.org/package=retractcheck) R package. With this package you can check large batches of DOI numbers to see if the corresponding articles have been retracted.
