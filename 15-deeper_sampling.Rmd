...
Unfortunately, most textbooks don't present $F$-values in the conceptually easy to follow way presented above. Instead, they use the algebraic rearrangement presented below. The rearrangment is important, however, because it is the basis for how information is presented in an ANOVA table. Unfortunately, when the rearranged in used - it's hard to understand the equation.
...
$$
\begin{aligned}
\text{total variance in means} &= \text{variance due to sampling error} + \text{variance in population means}\\
&= \frac{\sum_{i=1}^{a=3}{(\bar{x}_i - \bar{\bar{x}})^2}}{a-1}\\
&= \frac{(17498.97959-20899.83503)^2+(20196.90306-20899.83503)^2+(25003.62245-20899.83503)^2}{3-1}\\
&= 14450501.13
\end{aligned}
$$

Third we compare those two estimate in a ratio:

$$
\begin{aligned}
F &= \frac{\text{total variance in means}}{\text{variance due to sampling error}}\\
