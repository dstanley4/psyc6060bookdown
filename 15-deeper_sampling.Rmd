...
In the last chapter, we examined a scenario with multiple populations where the **population means were identical**. Consequently, sample means differed for one reason: random sampling error. This meant the formulas for a) total variance [observed variance] and b) random sampling variance [expected variance] each estimate the same underlying variance in sample means; because when population means are identical, variability arises solely from sampling error.

**In this chapter**, we examine a scenario with multiple populations where the **population means are different**.  Consequently, sample means will differ for two reasons. More specifically, sample means will differ due to differences among the population means and random sampling error. This means the formulas for a) total variance [observed variance] and b) random sampling variance [expected variance] estimate different parameters: the former reflects both sampling error and variability in population means, while the latter reflects only sampling error.

## Theory sample mean variance 

### Single population (theory)

Although the focus of this chapter is multiple populations, we begin with a single population. Recall that when there is a single population we express the variance in sample means using the formula below. This formula is a conceptual one that calculates the variance in sample means due to sampling error for a given population variance and sample size.
...
When there are multiple populations with identical variances, but different population means, that same formula applies for sampling error. That is, because all the populations have the same variance, we can use the old formula to calculate the variability in sample means due to sampling error. In ANOVA, we assume the various populations have equal variances (homoscedasticity). In this example, we know this assumption is true because we generate the populations ourselves with identical variances.
...
Importantly, if all populations have the same mean, then  $\sigma_{\mu_A}^2=0$. When this happens, sample means differ only due to sampling error. 
...
In this chapter, our final goal is to examine the multiple population scenario where $\sigma_{A}^2>0$. That is, the situation where the two formulas estimate different quantities because population mean differences contribute additional variance. But we begin with a review of the single population scenario.
...
Thus, where there is a single population (with variance 156.3236) and the sample size is n = 5, sample means will differ only due to sampling error. The theoretical variance of the sample means, based solely on sampling error, is 31.26472.
...
The trial column indicates which trial (of the 9,000) is represented in that row. The source population column indicate the population from which we obtained the sample (1 since we only had 1 population). The sample mean column indicates the sample mean in that trial.
...
We take the variance of all of those sample means and get a value of 30.80005 which closely approximates the actual value of 31.26472 (which we get when using an infinite number of sample means.) 
...
So the total variance in sample means is a combination of the variance in the population means and random sampling error:
...
The first few rows of the data set with 9,000 sample mean are shown below:

```{r}
head(sample_means)
```

The trial column indicates which trial (of the 9,000) is represented in that row. The source population column indicate the population we obtain the sample from (1 since we only had 1 population). The sample mean column indicate the sample mean in that trial.
...
We take the variance of all of those sample means and get a value of 297.7683 which closely approximates the actual value of 297.9314 (which we get when using an infinite number of sample means.)
...
In contrast, if the crowding manipulation did influence aggressive behavior, this means there are different population means for the three crowding conditions. If the population means are different, then the variance for the distribution of sample means would correspond to:
...
Because the red line is so extreme, the $p-value$ is very small $p<.0001$. **This is the probability of our data (or more extreme data), assuming the population means are all the same.**

Consequently, we say that the ANOVA is significant. We began by assuming the population means were all the same. Under this assumption, our study result is incredibly unlikely. Consequently, we reject the null hypothesis that the population means are equal and conclude that at least one population mean differs from the others.
...
|:----------------------------:|:----------------------------------------------------------------------------------------:|:---------------------------------------------------------------------------------:|:----------------:|
|      Method 1: Observed Variance      |  $\sigma_{\bar{x}}^2 = \lim_{K \to \infty} \frac{\sum_{i=1}^{K} (\bar{x}_i - \mu_{\bar{x}})^2}{K}$   |    $s_{\bar{x}}^2 = \frac{\sum_{i=1}^{a}{(\bar{x}_i - \bar{\bar{x}})^2}}{a-1}$    | Sampling Error & Variance of Population Means |
