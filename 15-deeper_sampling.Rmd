...
In contrast, the formula below estimate the variance due to sampling error alone:
...
Unfortunately, most textbooks don't present F-values in the conceptually easy to follow way presented above. Instead, they use an algebraic rearrangement presented below. This rearrangment is important, however, because it is the basis for how information is presented in an ANOVA table. Unfortunately, when rearranged in the manner below - it's hard to understand the equation.
...
Likewise, the calculation version is often presented in a different way:
