...
```{r notationdata, echo = FALSE, out.width="35%", fig.cap = "Data for understanding summation notation"}
...
**Calculating a mean**. The full version of the notation can be used to indicate how a mean is calculated.
...
It can be helpful to create a model that describes our data. Of course, the model won’t describe every participant perfectly. Each participant will differ to some extent from the model. The  difference between each participant and the model is called error.

Consider a model for our three cookies. The weight of a specific cookie is indicated by $X_i$. An estimate of that cookie's weight is indicated by the "model". The difference between the model of the cookie's weight and actual cookie's weight is indicated by $error_i$.
...
In addition to the original unit metric, however, it is also possible - and sometimes desirable to use the standardized mean difference. The word *standardized* is used to indicate that the comparison is relative to the standard deviation.
...
[@cohen1988] suggested that the standardized mean difference values of 0.20, 0.50, and 0.80 correspond to the effect size labels of small, medium, and large, respectively. These effect sizes are illustrated in Figure \@ref(fig:dex4). As described in this interesting  [blog post](https://replicationindex.com/2015/09/22/the-statistical-power-of-abnormal-social-psychological-research-a-revew-by-jacob-cohen/) these benchmark values came from reviewing a single issue of the *Journal of Abnormal and Social Psychology*. This was a relatively small number of studies - that were prone to publication bias. A recent investigation [see @schafer2019meaningfulness] of effect sizes in pre-registered studies, with no publication bias, suggest substantially lower benchmark values.
