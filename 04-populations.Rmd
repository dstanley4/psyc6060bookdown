...
* Populations are described using parameters

* Commonly used parameters include mean (mu or Xbar), variance (sigma2), or sd(sigma2)
...
* SMDis a common effect size for comparing means.

* SMD is based on the premise that the two population being compared have the same standard deviation

* SMD is indicated at the population level using the greek letter delta ($\delta$). At the sample level we tend to use the term "$d$-value".

* SMD, or Cohen's d, values represent the number of standard deviations between two population means
