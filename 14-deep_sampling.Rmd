...
Although we can just look the numbers resulting from these two methods and compare them, that's not how statisticians typically handle comparisons. The approach used by statisticians to compare two numbers is to create a ratio. That is, take one number and divide it by the other number. If the two numbers are the same the ratio will be 1.00.
...
### Method 1: Estimation using sample means

If we obtain a set of sample means from this population of sample means (i.e., distribution of sample mean) we can use those to estimate the variance of the distribution of sample mean as illustrated in the formula below. We use the lower case "a" to represent the number of sample means (in the case because we have 3 sample means $a = 3$). 
...
Second, we estimate the variance of the population of sample means ($s_{\bar{x}}^2$, see Figure 1B) using the set of sample means. Notice the use of "-1" in the denominator - [as we discussed previously in the sample accuracy chapter](https://dstanley4.github.io/psyc6060bookdown/sampling-accuracy.html#estimating-sigma2). The use of the "-1" in the denominator means we are not calculating the variance of these three sample means. Rather, we use -1 in the denominator we are estimating the variance of the population of sample means from which they were obtained (i.e., the distribution of sample means depicted in Figure 1B).
...
Thus, using Method 1, our estimate of the variance of the distribution of sample means is XXX.


### Method 2: Estimation using CLT shortcut

Recall, than an alternative to calculating the variance of the distribution of sample means is to use the shortcut formula based on the Central Limit Theorem (CLT). 
...
The problem with the formula above is that you need to know the variance of the all the people in the population ($\sigma_{people}^2$) - and we don't know the variance of all the people in the population. But, we do have three samples from that population of people. Because the three samples are from the same population of people - we know each sample provides an estimate of the same (single) variance of people in the population. Therefore, we can combine those variance estimates into better (or average) estimate of the variance of the people ($s_{people}^2$) in the population using the formula below.
