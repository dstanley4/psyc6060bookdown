...
Although each estimate was on average correct, the two estimates were not always the same. Recall we compared the two estimates by making a ratio. In the graph below you can see the range of values that occur with this ratio. 
...
ggplot(data = data_ratios, mapping = aes(x = var_ratio)) +
  ...
  ylab("Frequency") +
  ggtitle("Samples from one population") +
  theme_classic()
```
...
ggplot(data = data_ratios3, mapping = aes(x = var_ratio)) +
  ...
  xlab("Variance Ratio: (method_1_var / method_2_clt)") +
  ylab("Frequency") +
  ggtitle("Samples from three identical populations") +
  theme_classic()
```
