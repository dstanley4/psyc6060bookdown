...
It’s helpful, moving forward to, think of this formula ($\frac{\sigma_{people}^2}{n}$) as representing the variability due to random sample. No matter the situation - this formula will always reflect the variability in sample means due ONLY to random sampling.
...
Although we can just look the numbers resulting from these two approaches and compare them, that's not how statisticians typically handle comparisons. The approaches used by statisticians to compare two number is to create a ratio. That is one number is divided by the other number. If the two numbers are the same the ratio will be 1.00.  
...
In this section, we focused on actually **calculating** the variance of the distribution of sample means using two approaches. But to actually calculate this variance we needed an infinite number of sample mean (approach 1) or knowledge of the population variance (approach 2). Both are unrealistic approaches to determining this value.
