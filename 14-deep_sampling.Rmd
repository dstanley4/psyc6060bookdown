...
To begin, we are looking at a distribution of sampling means **where the means differ only due to random sampling error**. In this situation, we have calculated the variance of the distribution of sample means using two different methods.
...
$$
\begin{aligned} 
\text{ratio comparing methods} &= \frac{\text{total variance of sample means }}{\text{variance of sample means due to random sampling error}}\\
...
In this section, we focused on actually **calculating** the variance of the distribution of sample means using two approaches. But to actually **calculate** this variance we needed an infinite number of sample means (Approach 1) or knowledge of the population variance (Approach 2). Consequently, it's impractical to **calcualte** either quantity.
...
Likewise, recall that in Appraoch 2, we didn't know the variance of the population of people ($\sigma_{people}^2$) we had to rely on an esimate of that value ($s_{people}^2$), based on 15 people (three sets of 5). Consequently, Approach 2 will differ from the truth as well due to random sampling error.
