...
-   think of var/n as representing the variability in the distribution of sample means due to random sampling
-   the findings that we illustrate here only hold when there is only one population the sample means come from that one population.
...
We used two different approaches to estimating the variance of the distribution of sample means. With our "all knowing cap" on we can see, in the table above, that both methods produce values that differ from the actual variance of the distribution. That's because the best we can do with sample data is obtain an estimate of the variance of the distribution of means - we can never know the actual variance of the distribution of sample means.
