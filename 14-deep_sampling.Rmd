...
Since we are thinking of the distribution of sample means as a population we can calculate a mean a variance for the distribution of sample means.
The mean of the population of sample means is calculated as below. To make things a bit easier to follow we use $A$ instead of $N$ to represent the number of sample means (where $A$ = infinity, because there are an infinite number of sample means in the distribution)\footnote{I know it doesn't make sense the divided by infinity per se; however, in formal mathematics we would write this up in a different way describing the resulted value as an *expected value*.}.
