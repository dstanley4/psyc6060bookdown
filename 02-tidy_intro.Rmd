...
In this chapter we focus on teaching you foundational skills for data preparation. The skills taught in the this chapter large revolve around using the tidyverse packages to manage data that will eventually help with data cleaning in the [Workflow](https://dstanley4.github.io/psyc6060bookdown/an-emphasis-on-workflow.html) and [Qualtrics](https://dstanley4.github.io/psyc6060bookdown/qualtrics.html) chapters. There are some additional resources you may want to consult as you learn these tidyverse skills:
...
* You may find a [cheatsheet](https://www.rstudio.com/resources/cheatsheets/) helpful. Specifically, the [Data Transformation](https://github.com/rstudio/cheatsheets/raw/master/data-transformation.pdf) cheatsheet. 
...
After the remotes package is installed, it can be used to install the introverse package from GitHub (an alternative "App Store"). If you get ask if you would like to install more recent versions of packages on your machine press "1" and Return to indicate "All" (if you are asked, you may not be).
...
The **introverse** package is just additional documentation that is helpful for beginners - until you learn to read the standard R documentation. The first command we will use in this chapter is the **read_csv()** command. You get get the standard R help for this command using the code below in the R Console:

```{r, eval = FALSE}
library(tidyverse) # this actives the tidyverse
?read_csv
```
...
knitr::include_graphics("ch_tidy_intro/images/screenshot_R_help.png")
```
...
```{r, echo=FALSE, out.width="50%"}
knitr::include_graphics("ch_tidy_intro/images/screenshot_introverse_help.png")
```
...
If you inspect the Files tab on the lower-right panel in R Studio you will see the file data_okcupid.csv. The code below loads that file. Recall you should tpe this code into the R Console.
