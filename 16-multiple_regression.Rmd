...
Let's take a minute to consider the nature of the data we have so far.  For each person, we have three values: age, video, and video game score. If are to represent each person on a graph we need a 3D graph. Typically, we place 


Recall we previously conducted our regression with this command:
...
The lm_object has the three data point for each person (age, iq, video_game) embeded inside it. 


So we can use the make a 3D graph using the lm_object. We do so via the fast.plot command in the fastInteraction package.

Note:  The fast.plot command uses the argument "moderator" which is not appropriate in our context. When you see moderator in the command below just think of it as another predictor.
