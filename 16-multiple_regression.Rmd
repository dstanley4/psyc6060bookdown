...
we use a data set called  "data_mr_ex.csv". In this exqm
...
GO BACK AND MAKE 3D graph for two predictors in fastInteraction ..expand fast interaction
...
reg_with_raw_variables <- lm(video_game ~ iq + age,
                             data = my_data)
```
...
To obtain beta-weights there are two steps. First, we create standardized score versions of each column. Second, we run the usual regression using those columns.
...
```{r}
reg_with_standardized_variables <- lm(z_video_game ~ z_iq + z_age, 
                                      data = my_data)
...
For the **b*-weights* (far above), describe how a 1 unit change in the IQ influences video_game poinits. Similarly, beta-weights, describe how a 1 unit change in z_iq influences z_video_game. Keep in mind, however, that 1 unit of z_iq (and z_video_game) is 1 standard deviation. As a result, a beta-weight indicates how much the criterion scores will change in SD units when a predictor increases by 1 SD -- holding the effect of the other predictors constant. Similar to b-weights, beta-weight can only be interpreted in the context of the other variables in the equation.

One name for beta-weights is *standardized regression weights*. It would be more accurate, however, to  describe beta-weights as the **weights for standardized variables**.
