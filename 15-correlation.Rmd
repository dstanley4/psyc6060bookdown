...
Consider the following the scenario, we want to examine the relation between IQ and video games scores for people who live in the City of Guelph. Because we want to make conclusions about people who live in the City of Guelph we refer to this as our population. Next, imagine that we are actually able to get an IQ and a video game score for everyone in the City of Guelph (*N* = 100,000). That is we have data for everyone in the population.
...
* Correlation/regression can never provide evidence of causation (or explanation) but we can use a correlation to find a pattern in our data that is consistent with a causal relation. Then we conduct a second **experimental** study to determine if their really is a causal relation.
...
We can plot a variable (e.g., extraversion) against video score to see if there is a relation between the two. In this case there doesn't appear to be a relation. Indeed, this graph illustrates a $\rho = 0$ correlation between extraversion and video game scores. That is, there is no linear relation between extraversion and video game scores.


Below we place a horizontal line on the graph which indicates the mean Video Game Score. We also place a red best-fit line (i.e., regression line) on the graph; however, it is completely hidden by the black line representing the mean video game score. In this case, with these data, knowing a person's extraversion score **does not** allow us to provide an individualized estimate of a person's video game score. As a result, extraversion does not help us explain the variability in video game scores. That is, extraversion scores doesn't not allows us to explain why some people have high video game score whereas other people have low video game scores.
...
What if we were to try another variable -- like IQ? This illustrates a positive $\rho = .60$ correlation between IQ and video game score. That is, there is a linear relation between extraversion and video game scores and the strength of that linear relation is indexed by the population correlation $\rho = .60$. If there is a linear relation between two variables it simply indicate that the data cluster around a best-fit line when you draw it on the graph.

As before, we place a black horizontal line of the graph to indicate the population mean. Additionally, we place the a red regression line (i.e., a best-fit line) on the graph. This regression line represents an individualized estimate of video game score for each person based on their IQ. You can see that for some people the individualized estimate of video game score (i.e., the y-axis position of the red line) is higher than the population mean. That is, in some cases the red regression is line is higher than the black mean line. For other people, the individualized estimate of video game score (i.e., the y-axis position of the red line) is lower than the population mean. That is, in some cases the red regression is line is low than the black mean line. Overall, because the individualized estimates differ  from the population mean - we can suggest a reason why video game scores vary. Specifically, it appears that individuals with a high IQ tend to have a high video game scores whereas individuals with a low IQ tend of have a low video game score.
Later we see how to calculate $R^2$ which indexes the extent which the individualized estimate of video game score (i.e., the regression line) predict video game score above and beyond the population mean.
...
In the above diagram we illustrate the population-level correlation ($\rho$) and population-level slope of the regression line. Cohen (1988) suggest the effect size labels below. These effect size labels should be interpreted with caution. The magnitude of an effect is best considered in the context of the field of research and the consequences of an effect on individuals in both the short and long run.
...
Unfortunately, we never/rarely have data for everyone in the population (in this example everyone in the City of Guelph). Consequently, we usually have to select a subset of the population as a sample and use sample data for our analyses. We always need to remember the sample correlation ($r$) only provides an estimate of the population correlation ($\rho$). The sample estimate ($r$) might be "off a bit/a lot" from the population correlation ($\rho$) due to sampling error
...
We can look at just the sample data. Notice, in the figure below, that there is one plotted ($X_i$,$Y_i$) data point for each person.  As well we illustrate the sample mean with a horizontal green line.
...
The values used in the numerator of the **Sum of Squares Total** calculation are illustrated in the figure below. The vertical blue line indicates the difference between $Y_i$ and $\bar{Y}$. The $SS_{Total}$ value indexes the variability of the **actual** video game scores around the sample mean.
...
In the figure below we add the regression line in red. The regression line is a model for the data (a best-fit line). The regression line always runs through the joint mean of the two variables (i.e., the ($\bar{X}$,$\bar{Y}$) point).
...
Recall that the regression line represents and individualized estimate of each person's video game score derived from their IQ. You can see in the graph below that each person is represented by a blue dot. The estimate of each person's video game score, $\hat{Y_i}$, derived from the regression, is indicated by a red dot on the red regression line. The vertical blue lines are used to indicate for each person (i.e., blue dot, $Y_i$) the corresponding estimate of their video game score (i.e., red dot, $\hat{Y_i}$).

```{r, echo = FALSE, out.width="80%"}
knitr::include_graphics("ch_correlation/images/sample_plot4.png")
```

The regression line allows us to make an individualized estimate of the video game score for each person, $\hat{Y_i}$, based on our regression equation. We can calculate the extent to which individualized estimates are better than the sample mean in terms of modeling the data. We begin by calculating the extent to which the individualized estimates differ from the sample mean. This is done with the calculation below.
...
\end{align}
$$
The values used in the numerator of the **Sum of Squares Regression** calculation are illustrated in the figure below. The vertical red line indicates the difference between $\bar{Y_i}$ and $\bar{Y}$. The $SS_{Regression}$ value indexes the variability of the **estimates** of video game scores around the sample mean. The longer the vertical red line (i.e., the larger the $(\hat{Y_i} - \bar{Y})$ difference) the better the model. The length of the vertical red line is an index of the variability in scores accounted for by the model. Longer vertical red lines are associated with models that do a better job of accounting for variability in video scores.
...
We can obtain the regression model (i.e.. linear model or lm) using the command below:
...
If we examine the fit column on the far right we see the $R^2$ value is .74 indicating the 74% of the variability in video scores is accounted accounted for by the statistical model (i.e., red regression line).
...
The part of the exercise involves comparing correlations within and across studies. We begin by obtaining a data set from the psych package. Note that we **do not** use the library(psych) command due to conflicts with the tidyverse.
...
As per the correlations tables above we find the correlations for men ($r_{(a1, e1)}$ = 0.1387) and women ($r_{(a1, e1)}$=  0.1387) in the output. We also see the comparison: Difference: r1.jk - r2.hm = 0.0765. Looking below we see the zou2007 confidence interval.
