...
1) Calculate the grand mean (i.e., $\mu_A$). That is, the mean of the two juggling population means. Recall because there are two levels of juggling (i.e., two populations), $a = 2$.
...
1) We ESTIMATE the grand mean using the sample-level means. We refer to our grand mean estimate using $\bar{bar{bar}}$. There are two bars about the $x$ to indicate that it is the mean of means.
...
2) Now estimate the variability in populations means:
...
Unfortunately, when you use sample-level information to estimate population-level variance the number you calculate with reflect two things.

1) The variance of the population means $\sigma_{\mu_A}^2$
2) Random sampling error variance: $\frac{\sigma_{people}^2}{nb}$
...
Recall that we now have two estimates for the variability in juggling means.
...
#### $A$ typical (juggling)
  ...
  Juggling| Formula | Expected value | degrees of feedom |
:--------------:|:--------------:|:------------------:|:---------------:|
Sample size multiplied by estimated total variance| $\frac{nb\sum(\bar{x_i}-\bar{\bar{x}})^2}{a-1}$ |$\sigma_{people}^2 + nb \sigma_{A}^2$ | $(a-1)$
Estimated population variance | $s_{people}^2$ |$\sigma_{people}^2$ | $ab(n-1)$
Comparison using $F$-ratio | $\frac{\frac{nb\sum(\bar{x_i}-\bar{\bar{x}})^2}{a-1}}{s_{people}^2}$ | $\frac{\sigma_{people}^2 + nb \sigma_{A}^2}{\sigma_{people}^2}$ | $[(a-1),ab(n-1)]$
