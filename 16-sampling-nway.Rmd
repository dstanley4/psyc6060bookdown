...
s_{\mu_A}^2 &=  \frac{\sum (\bar{x}_{A_i}-\bar{\bar{x}})^2}{a-1} \\
&=  \frac{(\bar{x}_{A_1} - \bar{\bar{x}})^2 + (\bar{x}_{A_2} - \bar{\bar{x}})^2}{a-1} \\
&=  \frac{(65.4 - 66.56667)^2 + (62.6 - 66.56667)^2 + (71.7 - 66.56667)^2}{2-1} \\
&= 21.72333
\end{aligned}
$$
...
Summarized below:
  ...
  Preparation| Formula | Expected value | degrees of feedom |
:--------------:|:--------------:|:------------------:|:---------------:|
Sample size multiplied by estimated total variance| $\frac{na\sum(\bar{x_i}-\bar{\bar{x}})^2}{b-1}= 217.2333$ |$\sigma_{people}^2 + na \sigma_{B}^2$ | $(b-1)$
Estimated population variance | $s_{people}^2=60.0$ |$\sigma_{people}^2$ | $ab(n-1)$
...
When we say two variable **interact** to predict the dependent variable we are saying there is more going on that the two main effects.
So when we seek to determine if there is an interaction we work with **residualized** cell means. That is, cell means were the main effects have been removed. In the next section we illustrate how those residualized cell means are calculated.
