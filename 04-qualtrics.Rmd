...
Read the file in with the right columns names - excluding unwanted rows.
...
Recall that when we obtained our data from Qualtrics we selected "Use Choice Text. That means that, as desired, we didn't get values for our Likert items - we obtained labels. Now we need to switch the Likert responses to numeric values. The advantage of handling our data in this way is that anyone examing our data and code in an open access repository can see exactly what options survey particpants were provided with for each question. 
...
N <- dim(analytic_data_survey)[1] # get the number of rows in the data set
