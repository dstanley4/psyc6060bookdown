...
3. Click the Export Question Tags check box. You will see the default names for the items (instead of the ones you indicated in the codebook). Clock the Close button in the lower right when you're done.
...
* Rename the file to data_survey.csv
...
2. Drag the data_survey.csv file to the survey_data folder.
...
* Remove some, or all, of the additional columns on the side of data set added by Qualtrics
...
# skip the first 3 rows than read in the data and use names from above step
...
In the command below, we ensure our column names follow the tidyverse [style guide](https://style.tidyverse.org) by using the clean_names() command from the janitor package. Addditionaly, we remove empty row and columns from the data using the remove_empty_cols() and remove_empty_rows(), respectively.  
...
When you have type chr for a column the computer just see the context of each column as text that is different for everyone. It doesn't even realize that two people who both responded "Slightly Agree" responded the same way. It sees every response in the column as unique. Check that out for the aff_com1_likert7 item.
...
To convert a column from txt to fct we need know the exact labels used in that column. We can obtain those with the code below. In this code, we use just one item to determine the response options the aff_com1_likert7 item. We use the pull() command to obtain all the participant responses from that column. Then we use the unique() command to give use just one instance of each response.
...
Now the we know the labels used in that column (and all the columns that end in likert7) we create an ordered version of those labels with the code below. I copied and pasted the text (e.g., "Strongly Disagree") from the above output to avoid mistakes.

```{r}
# write code to create a factor
...
Now turn each column of text (i.e., type chr) into a factor using level labels we created above we use the code below. 
...
You can see they are numeric values with the code below. Notice the column types for likert7 items is now dbl (short for double - a type of high precision number.)
...
```{r}
# Create numeric versions of Likert Response Options
# First mutate command turns everything to a factor as defined above
# Second mutate command converts factor to a numeric variable
# Treat 5 and 7 point scale in different code blocks
analytic_data_survey <- analytic_data_survey %>%
...
A commplete discussion of the rationale for flipping reverse-key (i.e., reverse worded) items is provide the previous chapter in the [Scale Scores](https://dstanley4.github.io/psyc6060bookdown/an-emphasis-on-workflow.html#scale-scores) section. We use that logic in the script below.
