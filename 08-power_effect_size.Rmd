...
Another approach to determine the effect size using in your power / sample size analysis is to determine the smallest effect that would have been significant in the previous study [@lakens2018equivalence]. We do that with the code below:
...
To determine a sample size for you need to specify:
* an estimate of the population effect size (see the various methods reviewed above)
* desired power (probability of a significant effect if it exists)
...
* Effect size estimate from the from the small telescope approach ($\delta = .24$)
...
But what happens to our power if we get a greater/fewer number of participants? We don't always have control over the exact number due to non-response rates etc. We can use the graph below to see how power changes as the number of participants changes. Based on this graph, we may want to adjust the number of participants.
...
power_labels <- c("red: dangerously low power (0 to .50)",
                  "light red: low power (.50 to .80)",
                  "green: adequate power (.80 to .95)",
                  "dark green: almost certain power (> .95)")
...
Let's assume that after looking at all the graphs and tables you end up sticking with your intial sample size of 298 per group. What sample-level effect sizes will be significant when you do so?
...
This indicates that with a sample size of 298 per group that only $d$-values greater than $d$ = 0.135 will be significant. 
