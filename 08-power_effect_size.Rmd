...
Thus, $d$ = 0.45, 95% CI [0.05, 0.85]. Therefore, with a sample $d$-value of 0.45 ($n_1=n_2=50$) a plausible range for the population $d$-value ($\delta$) is .05 to .85. That is, the population $d$-value ($\delta$) could be as low as .05. With the safeguard approach you use $\delta = .05$ as your population $d$-value in the power/sample size analysis.


#### Small telescope 

The small telescope approach to determining the effect size to use if you power/sample size analysis is based on the analogy of a telescope [@simonsohn2015small]. Imagine you are an astronomer. A previous study used a small telescope and claimed to see a new asteroid. You are the proud owner of a large telescope and decide to look for the asteroid. If the asteroid exists then you should be able to find it easily with your larger telescope. Conversely, if the you are unable to find the asteroid with your larger telescope it draws into question the original study. In this example, the size of the telescope is a proxy for sample size.

We treat a previous study as a small telescope - a reasonable assumption given what we know about power levels in psychology studies. Specifically, we begin by de
...
red_zone <- pwr.t.test(n = n, power = .50)$d
green_zone <- pwr.t.test(n = n, power = .80)$d
green_zone2 <- pwr.t.test(n = n, power = .99)$d
library(tidyverse)
power_plot <- ggplot(data = power_values,
  ...
  labs(title = sprintf("Power plot for N = %g", n),
       ...
       y = "Power (probability of sig. effect)") +
  annotate("rect", xmin = 0, xmax = red_zone, ymin = 0, ymax = 1, fill = "red", alpha = .2) +
  annotate("rect", xmin = red_zone, xmax = green_zone, ymin = 0, ymax = 1, fill = "yellow", alpha = .2) +
  annotate("rect", xmin = green_zone, xmax = green_zone2, ymin = 0, ymax = 1, fill = "green", alpha = .2) +
  annotate("rect", xmin = green_zone2, xmax = max_pop_d, ymin = 0, ymax = 1, fill = "green", alpha = .5) +
  theme_classic()

print(power_plot)

# table
power_table <- data.frame(power = c("dangerously low (0 to .50)", "moderate (.50 to .80)", "high (.80 to .99)", "almost certain (> .99)"),
                          pop_d_start = round(c(0, red_zone, green_zone, green_zone2),2),
                          pop_d_end = round(c(red_zone, green_zone, green_zone2, NA),2))
