...
png(filename = "ch_samples_precision/images/dskew.png", width = 12*300, height = 9*300, res = 300)
gridExtra::grid.arrange(dp10, dp200)
dev.off()
```

```{r dvalueskew, echo = FALSE, out.width = "80%", fig.cap = "d-value skew graphs"}
knitr::include_graphics("ch_samples_precision/images/dskew.png")
```
...
There is a standard error for $d$-values (i.e., $SE_d$ or $SE_{d_{unbiased}}$). It is simply the standard deviation of the thousands of $d$-values in the simulation. More generally, it is an index of much $d$-values vary from a population standardized mean difference ($\delta$) due to sampling error for a given sample size.


**The precision of sample $d$-values is influenced by:**

* population standardized mean difference ($\delta$)
