...
### Cheatsheets
As you learn R and RStudio you should be aware there are a number of [cheatsheets](https://www.rstudio.com/resources/cheatsheets/) available. There is even one for the [RStudio interface](https://github.com/rstudio/cheatsheets/raw/master/rstudio-ide.pdf).
...
Before you can use a package it needs to be installed -- this is the same as downloading an app from the App Store. Normally, you can install a **single** package with the install.packages command. Previously, you needed to run an install.package command for every package in the tidyverse as illustrated below (though we no longer use this approach).
