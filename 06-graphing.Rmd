...
Color blindness is an issue that affects approximately five percent of the population. A nuanced discussion of the different types of color blindness is beyond the scope of the current discussion. We can, however, take a momement to think about every graph as being composed of two parts that work together to create the overall image. We can can there of there being a lightness (i.e., light vs. dark) component and a hu component. Together these two components work together to create the image that we see. This distinction between lightness and hue is relevant to all images [see @margulis2005photoshop].  

```{r colorcomparison, out.width= "85%", echo = FALSE, fig.cap = "Color information removed from current graph"}
...
We can use the color palette by specifying it's name within scale_fill_brewer(). We want to use the Paired palette in Figure \@ref(fig:colorspalthree) so we use the code: scale_fill_brewer(palette = "Paired"), as illustrated below:
...
The palette above use the first two colors of a three color palette. What if you wanted the second two colors in the palette? If so, we obtain the colors for each level of the color palette with the command below:

```{r}
brewer.pal(n = 3, name = "Paired")
```

These numbers correspond to values representing the three colors of the Paired palette. If we want the last two colors in the palette for our graph that means we want the colors: #1F78B4 and #B2DF8A.
...
We can put the two colors we want (#1F78B4 and #B2DF8A) on the graph by once again using the scale_fill_manual() command - instead of the scale_fill_brewer() command. We can see this in the code below:
...
Now that we have used a color-blind palette we can look at the graph without the color component again. We see in Figure \@ref(fig:colorcomparison2)B, below, that when we examine bars based on lightness we now have contrast. The new color-blind safe color vary in term of hue and lightness - not just hue. Therefore, when the hue is removed the two colors are distinguishable -- and safe for color-blind individuals.

```{r colorcomparison2, out.width= "85%", echo = FALSE, fig.cap = "Color information removed from new color-safe graph"}
