...
Next, we want to put the boxoffice revenue about each bar so it eaiser to interpret. In R terms, we are putting a label above each bar. We want the contents for the labels to come from the boxoffice column. Therefore, we add the geom_text() command below:
...
```{r, showmap, echo = FALSE, out.width = "70%", fig.cap="ggplot()'s internal data."}
...
Fortunately, if we simply put "mapping = aes(label = tomatoes_aud)" within the new geom_text() command we get the desired information on the graph. The mapping/aes argumentrs within geom_text() overrides the mapping/aes argumentrs within ggplot(). Or more accurately, the new geom_text() command creates it's own version of the internal data set in which the label column is filled with information from tomatoes_aud.
