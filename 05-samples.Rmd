...
We are interested in sample means ($\bar{x}$) to the extent that they provide an estimate of the population mean ($\mu$). The population mean is calculated using Formula \@ref(eq:popmeanch5):
...
We are interested in sample variances ($s^2$) to the extent that they provide an estimate of the population variance ($\sigma^2$). The population variance is calculated using Formula \@ref(eq:popvarch5):
...
Because we rarely has access to data from an entire population we want to estimate the population variance using the sample data. However, for variance, estimating the population parameter from a statistic, is more complicated than it was for the mean.

we use the sample mean as an estimate of the population mean. The sample mean, $\bar{x}$, is a statistic calculated using the using Formula \@ref(eq:samplemeanch5). The bar of above the $x$, indicates that it is a mean. Notice that Formula \@ref(eq:popmeanch5) and Formula \@ref(eq:samplemeanch5) are the same - even though they use different notation.
...
The values in the sample.var.n_1 column  were calculated, based on sample data, using Formula \@ref(eq:samplevarn1ch5). Check out this column using the head() command:

```{r}
head(many_samples, 10)
```

We can evaluate the quality of Formula \@ref(eq:samplevarn1ch5), using $n-1$, by averaging over values in the sample.var.n_1 column.
