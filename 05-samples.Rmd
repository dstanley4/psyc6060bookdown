...
Sample-level correlations, $r$, often differ from the population-level correlation (i.e., $\rho$) due to sampling error. We can confirm that sample correlations are not substantially biased with a simulation. That is, we can confirm that the average of many sample correlations ($r$) roughly equals the population correlation (i.e., $\rho$) using a simulation. We say "roughly equal" because $r$ is a biased estimator of $\rho$ but the bias is typically small enough that it can be ignored (Schmidt & Hunter, 2015),
...
The glimpse() command reveals there are 300000 rows and two columns (weight and height).
...
We see from this matrix that the correlation between weight and height is $\rho$ = .50 (with rounding).

To examine the extent to which this population correlation, $\rho$ = .50, is estimated by the sample statistic, $r$, we need to take a larger number of samples. Therefore, we take 50000 sample (each n = 75) and calculate the correlation for each of them:
...
We can examine the contents of many_samples using the head() command. Each row of many_samples illustrates a sample correlation in the $r$ column. If you examine the first row carefully you see that the data in this row corresponds to the hand calculation example.
...
1 - add correlation sampling graphs etc. emphsize subset of row so population correlation vs sample correlaiton is clear...

2- emphaise SMD is scenario where we assume equal population variances
