...
pop_graph <- ggplot(data = pop_data,
  ...
  annotate(geom = "text", x= 220, y = 90, label = rhopop, parse = TRUE) +
  labs(title = sprintf("Population correlation = .50, N = %g", N)) +
  theme_classic()
...
r_graph <- ggplot(data = many_samples,
  ...
  annotate(geom = "text", x= .65, y = 3, label = "Each r based", hjust = 0) +
  annotate(geom = "text", x= .65, y = 2.75, label = "on n = 75", hjust = 0) +
  labs(title = sprintf("%g sample correlations", N),
...
png(filename = "ch_samples/images/rdist_example.png",
    ...
    height = 8*300,
    res = 300)
gridExtra::grid.arrange(pop_graph, r_graph)
dev.off()

```


```{r rdistplot, fig.cap="Variability in sample correlations", echo = FALSE, out.height="80%", out.width="80%"}
knitr::include_graphics("ch_samples/images/rdist_example.png")
```
