...
We see from the output above that the average of the sample.var.n column is 141.86; which differs from the population variance of 157.46. Thus, the average of the sample variances is lower (141.86) than the actual population variance (157.46). This finding indicates that the sample variance, when calculated with $n$ in the denominator, is a biased estimate of the population variance. That means, that if we try to estimate the population variance with the sample variance that we will, on average, be wrong.


Fortunately, there is a way around the fact that the sample variance provides a biased estimate of the population variance. We can use a mathematical proof, see Hayes, to show than we can use sample data to estimate the population variance in an unbiased way. The results of the math is rather straightforward. We simply divide by $n - 1$ instead of $n$ when calcualting 
