...
Because we rarely have access to data for an entire population we typically want to estimate the population standardized mean difference using sample data. Many researcher esimate the population standardized mean difference from sample data using the Formula \@ref(eq:samplesmdch5) below. This value is known by many other names: $d$, Cohen's $d$, and Hedges $g$. Notice that the sample-level formula below, Formula \@ref(eq:samplesmdch5), is the same as the population level formula, Formula \@ref(eq:popsmdch5), only the notation differs.
...
Let's consider some hypothetical sample data to make the situation clear. More specifically, examine the sample-level statistics below for males and females. In particular not that we have two standard deviations -- one for males and one for females. Moreover, these two sample-level standard deviation (using $n$-1)  are not the same - they are different from each other. This initially seems problematic calculation of standardized mean difference requires that population standard deviaitons are identical. Fortunately, it's not a problem. 
...
We need to recognize that the sample standard deviations are likely to differ from the population standard deviation due to sampling error. Consequently, we are likely to get two different sample standard deviations even if the population standard deviations are the same for male and females.

How do we resolve this situation of having two sample standard deviations? The first step is to switch to thinking in terms of variance rather than standard deviation. Due to the way the math works - life becomes very complicated very quickly if we continue to think in terms of standard deviations. Let's reframe the problem into a variance problem. Variances are preferable to standard deviations because we can add and subtract variances - but not standard deviations.
...
If we apply this approach to the data in our example:
