...
In this particular scenario, your planned directional comparison involves comparing cell 1 (yes juggling/low preparation) with cell 3 (yes juggling / high preparation). You believe the mean for cell 3 will be higher than cell 1. This belief is based on theory and your registered it at [osf.io](osf.io) prior to collecting data. 
...
Note that when we conduct a $t$-test the estimate of the population variance of calmness scores ($ s_{people}^2$) is based on only the two cells involved in the comparison (see below). Recall though that in the ANOVA this estimated was based on all six cells.
...
Divide the two-sided $p$-value by 2 to obain the one-sided $p$-value.
...
Notably, we are missing an effect size, $d$-value with confidence interval, for the above.
...
Although the t-test is a popular approach for comparing two cell means in an ANOVA, a **contrast** is actually more consistent with the underlying ANOVA framework. The big difference between a t-test and a contrast is how the calculate the pooled variance term. In fact, that's the only difference. With a t-test the population variance of people's calmness scores is estimated using only the two cells involved in the comparison. The contrast, in contrast, takes a different approach. Even though only two means are being compared in a contrast, the  population variance of people's calmness scores is estimated using all the cells in the ANOVA.
...
Notice how the calculation of this $F$-value below follows the general logic we have used with $F$-values all along. The estimate of the total variance is divided by the estimate of the random sampling variance.

We begin by refering to the means we will use in the contrast as contrast means.
...
When we look at these two contrast means, the mean of the means is:
...
We calculate variance due to random sampling:

$$
\frac{s_{people}^2}{n} = \frac{MSE}{n} = \frac{60}{5} = 12  
$$

And then we compare them with the F-ratio:
...
When it is rearranged, you can see the $F = t^2$. This rearrangement only works when there are two means. The important point is though that you can see that really the $t$-test and the contrast are the same except for the way the pooled variance term is calculated.


We can calculate the contrast with the code below.
...
We can calculate an effect size with a CI for this comparison using the formula below.
 ...
 $$
 sc = \frac{\bar{x_3} - \bar{x_1}}{s_{people}}
 $$

Again, notice the only difference from a d-value is that we calculated  $s_{people}$ based on 6 cells instead of 2. Consequently, some people call this value a $d$-value.
...
Among jugglers, high preparation students ($M = 88.00$, $SD = 8.15$) were calmer ($M_{\text{diff}} = 16.6$, $d = 2.14$, 95% $CI[0.77, 3.49$]) than low preparation students,($M = 71.4$, $SD = 5.50$), $F(1, 24)= 11.482$, $p = .001$.
...
&= \frac{\frac{(\bar{x_1} - \bar{x_2})^2}{2}}{\frac{s_{people}^2}{n}}; \text{So we can change the numerator}\\
...
But when we look at these two contrast means, the mean of the means is:
...
We calculate the variance due to random sampling:
...
```{r}
xbar1 <- mean(juggling_low_preparation, na.rm = TRUE)
xbar2 <- mean(juggling_medium_preparation, na.rm = TRUE)
xbar3 <- mean(juggling_high_preparation, na.rm = TRUE)

c1 <- xbar1
c2 <- mean( c(xbar2, xbar3))
c_means = c(c1, c2)

total_var = var(c_means)

n <-  5
sampling_var1 <- mse/n
sampling_var2 <- mse/(n + n)
sampling_var = mean( c(sampling_var1, sampling_var2))
...
means_to_compare = c(xbar1, xbar2, xbar3)
