...
In the last chapter we focused on the ANOVA itself. In this chapter we focus on comparing the means within cells of the ANOVA. These comparisons could be:
...
A contrast differs from a t-test primarily because of how the pooled variance term is calculated.

In a contrast we use all 6 cells from the ANOVA to estimate $s_{people}^2$.
...
Notice how the calculation of this $F$-value below follows the general logic we have used with $F$-values all along. The estimate of the total variance is divided by the estimate of the randon sampling variance.

We calculate the total variance:
...
&=  \frac{(\bar{c_{1}} - \bar{\bar{x}})^2 + (\bar{c_{2}} - \bar{\bar{c}})^2}{2-1} \\
\end{aligned}
$$
